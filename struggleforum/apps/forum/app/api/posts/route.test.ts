...
describe("/api/posts route", () => {
  ...
  it("returns paginated posts", async () => {
    ...
    const res = await GET(
      new NextRequest(`http://localhost:3000${apiRoutes.posts.collection}`),
    );
  ...
  it("returns 400 for invalid query params", async () => {
    const res = await GET(
      new NextRequest(
        `http://localhost:3000${apiRoutes.posts.collection}?page=0`,
      ),
    );
  ...
  it("creates a post for the current session user", async () => {
    ...
    const req = new NextRequest(
      `http://localhost:3000${apiRoutes.posts.collection}`,
      {
        method: "POST",
        body: JSON.stringify({
